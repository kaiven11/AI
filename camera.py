    ...
    def read(self):
        ...
        if self.video_file:
            ...
            if self.copy_frame:
                ...
                copyframe = self.img_handle.copy()
                temp = copyframe.shape

                # print("current H:%d"%temp[0])
                # print("current W:%d"%temp[1])
                ...
                if self.cope: #用于切割
                    ...
                    asptio_x = temp[1] / raw_x
                    asptio_y = temp[0] / raw_y
                    x1*= asptio_x
                    y1*= asptio_y
                    h *= asptio_x
                    w *= asptio_y
                    copyframe = copyframe[int(x1):int(h)+int(x1),int(y1):int(w)+int(y1)]
                
                    cv2.imwrite('ttttt.jpg', copyframe)
                return copyframe # 防止丢帧
